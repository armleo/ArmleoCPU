...
class Cache(val c: CacheParams = new CacheParams, val verbose: Boolean = true, instanceName: String = "inst$") extends Module {
  ...
  val ways_width = log2Ceil(c.ways)
  ...
  val s1 = IO(new Bundle {
    ...
    val response              = Output(new Bundle {
      val bus_aligned_data    = Vec(c.ways, UInt(8.W))
      val miss                = Bool()
    })
  })
  ...
  // cache_ptag_width = apLen - log2Ceil(c.entries * c.entry_bytes) = 34 - log2(16 * 64) = 24 bits
  // entry num width = log2Ceil(c.entries) = 4
  // s0_entry_bus_num width = log2Ceil(c.entry_bytes / c.bp.data_bytes) = 1
  ...
  val s0_entry_bus_num  = s0.vaddr(log2Ceil(c.entry_bytes) - 1, log2Ceil(c.bp.data_bytes))
  val s0_entry_num      = s0.vaddr(log2Ceil(c.entries * c.entry_bytes) - 1, log2Ceil(c.entry_bytes))
  // val s0_cache_vtag     = s0.vaddr(c.avLen - 1, log2Ceil(c.entries * c.entry_bytes)) Do we even need this?
  ...
  // ways * c.entries * c.entry_bytes bytes of cache
  
  // Q: Why is the c.entries * c.entry_bytes limited to 4KB (or single page)
  ...
  class cache_meta_t extends Bundle {
    val valid = Bool()
    val cptag = UInt(c.cache_ptag_width.W)
  }
  ...
  val meta    = SyncReadMem(c.entries, Vec(c.ways, UInt((new cache_meta_t).getWidth.W)))
  val data    = Seq.tabulate(c.ways) {
    f:Int => SyncReadMem(c.entries * c.entry_bytes / c.bp.data_bytes, Vec(c.bp.data_bytes, UInt(8.W)))
  }

  val meta_rdwr = meta(s0_entry_num)
  val data_rdwr = Seq.tabulate(c.ways) {
  ...
  when(s0.cmd === cache_cmd.write) {
    ...
    meta_write.cptag := s0.writepayload.paddr(c.archParams.apLen - 1, log2Ceil(c.entries * c.entry_bytes))
    meta_rdwr(s0.writepayload.way_idx_in) := meta_write.asUInt
    //meta_rdwr(s0.writepayload.way_idx_in).valid := s0.writepayload.valid
    //meta_rdwr(s0.writepayload.way_idx_in).cptag := s0.writepayload.paddr(c.apLen - 1, log2Ceil(c.entries * c.entry_bytes))
    log("Write cptag/valid way: 0x%x, entry_num: 0x%x, entry_bus_num: 0x%x, cptag: 0x%x, valid: 0x%x", s0.writepayload.way_idx_in, s0_entry_num, s0_entry_bus_num, s0.writepayload.paddr(c.archParams.apLen - 1, log2Ceil(c.entries * c.entry_bytes)), s0.writepayload.valid)
  ...
  //    System a little bit
  val meta_read  = VecInit.tabulate(c.ways) {
    way: Int => meta_rdwr(way)
  }
  val data_read   = VecInit.tabulate(c.ways) {
