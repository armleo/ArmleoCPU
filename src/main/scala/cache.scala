...
class Cache(verbose: Boolean = true, instanceName: String = "inst$", cp: CacheParams = new CacheParams) extends Module {
  ...
  val s0 = IO(new Bundle {
    ...
    val vaddr       = Input(UInt(cp.archParams.avLen.W))
    ...
    val writepayload = new Bundle {
      ...
      val paddr             = Input(UInt(cp.archParams.apLen.W))
      val bus_aligned_data  = Input(Vec(cp.bp.data_bytes, UInt(8.W)))
      val bus_mask          = Input(Vec(cp.bp.data_bytes, Bool()))
  ...
  val s1 = IO(new Bundle {
    ...
    val response              = Output(new Bundle {
      val bus_aligned_data    = Vec(cp.ways, UInt(8.W))
      val miss                = Bool()
    })
  })

  val cycle = IO(Input(UInt(cp.lp.verboseCycleWidth.W)))
  val log = new Logger(cp.lp.coreName, instanceName, verbose, cycle)
  ...
  // Q: Why is cp.bp.data_bytes limited to 32 bytes?
  ...
  // s0_entry_bus_num width = log2Ceil(cp.entry_bytes / cp.bp.data_bytes) = 1
  // inbus_offset = log2Ceil(cp.bp.data_bytes) = 5
  // 5 + 1 + 4 + 24 = 34
  // For virtual address that is 32/22 bits respectively. But we need to use the physical address for comparison, anyway

  // val s0_inbus_offset   = s0.vaddr(log2Ceil(cp.bp.data_bytes) - 1, 0) Do we even need this?
  val s0_entry_bus_num  = s0.vaddr(log2Ceil(cp.entry_bytes) - 1, log2Ceil(cp.bp.data_bytes))
  ...
  // Q: Why are the data cp.bp.data_bytes long? Why not xLen long?
  ...
  class cache_meta_t extends Bundle {
    val valid = Bool()
    val cptag = UInt(cp.cache_ptag_width.W)
  }
  ...
  val data    = Seq.tabulate(cp.ways) {
    f:Int => SyncReadMem(cp.entries * cp.entry_bytes / cp.bp.data_bytes, Vec(cp.bp.data_bytes, UInt(8.W)))
  }
  ...
  when(s0.cmd === cache_cmd.write) {
    ...
    meta_write.cptag := s0.writepayload.paddr(cp.archParams.apLen - 1, log2Ceil(cp.entries * cp.entry_bytes))
    ...
    log("Write cptag/valid way: 0x%x, entry_num: 0x%x, entry_bus_num: 0x%x, cptag: 0x%x, valid: 0x%x", s0.writepayload.way_idx_in, s0_entry_num, s0_entry_bus_num, s0.writepayload.paddr(cp.archParams.apLen - 1, log2Ceil(cp.entries * cp.entry_bytes)), s0.writepayload.valid)
    ...
    for (way <- 0 until cp.ways) {
      ...
      when(s0.writepayload.way_idx_in === way.U){
        for(bytenum <- 0 until cp.bp.data_bytes) {
          when(s0.writepayload.bus_mask(bytenum)) {
            data_rdwr(way)(bytenum) := s0.writepayload.bus_aligned_data(bytenum)
            log("Write data way: 0x%x, entry_num: 0x%x, entry_bus_num: 0x%x, bytenum: 0x%x, data: 0x%x", way.U(ways_width.W), s0_entry_num, s0_entry_bus_num, bytenum.U(cp.bp.data_bytes.W), s0.writepayload.bus_aligned_data(bytenum))
          }
        }
      }
    }
  ...
  /* Output logic                                                           */
  /**************************************************************************/
  val s1_cptag = s1.paddr(cp.archParams.apLen - 1, log2Ceil(cp.entries * cp.entry_bytes))
