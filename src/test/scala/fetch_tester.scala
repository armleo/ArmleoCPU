...
class FetchSpec extends AnyFlatSpec {
  ...
  it should "Basic Fetch functionality test" in {
    ...
    simulate(new Fetch(c)) { dut =>
      dut.clock.step(Math.max(c.icache.entries * c.icache.entry_bytes / ccx.busBytes, c.itlb.entries)) // Flush
      ...
      for(i <- 0 until 4) {
        ...
        dut.uop_valid.expect(false.B)
        dut.ibus.ar.valid.expect(false.B)
        dut.ibus.r.valid.poke(true)
        dut.ibus.r.bits.data.poke(BigInt(f"F${i}E${i}D${i}C${i}B${i}A${i}9${i}8${i}7${i}6${i}5${i}4${i}3${i}2${i}1${i}0${i}", 16))
        if(i == 3)
          dut.ibus.r.bits.last.poke(true)
        dut.clock.step(1)
      }
      dut.uop_valid.expect(false.B)
      dut.ibus.r.valid.poke(false)
      dut.ibus.r.bits.last.poke(false)
      dut.ibus.r.bits.data.poke(BigInt("DEADBEEFDEADBEEFDEADBEEFDEADBEEF", 16))
      dut.clock.step(1)

      dut.uop_valid.expect(true.B)
