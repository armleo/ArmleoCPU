...
class FetchSpec extends AnyFreeSpec with ChiselScalatestTester {
  ...
  "Basic Fetch functionality test" in {
    ...
    test(new Fetch(c)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      dut.fetch_uop_valid.expect(false)
      dut.clock.step(1)
      dut.ibus.ar.valid.expect(true)
      dut.ibus.ar.addr.expect(c.reset_vector)
      ...
      dut.fetch_uop_valid.expect(true)
      dut.fetch_uop.pc.expect(c.reset_vector)
      dut.fetch_uop.instr.expect(BigInt("030201000", 16))
      dut.fetch_uop_accept.poke(true)
      dut.clock.step(1)

      dut.fetch_uop_valid.expect(true)
      dut.fetch_uop.pc.expect(c.reset_vector + 4)
      dut.fetch_uop.instr.expect(BigInt("070605040", 16))
      dut.fetch_uop_accept.poke(true)
      dut.clock.step(1)


      dut.fetch_uop_accept.poke(false)
